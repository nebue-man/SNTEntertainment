...
// position:fixed is positioned relative to the viewport UNLESS an ancestor
// establishes a containing block (transform, perspective, filter, will-change,
// or contain). Detect that block so we can compensate the translate offset.
...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    if (hideDefaultCursor) document.body.classList.add('custom-cursor')
    ...
    const scrollHandler = () => {
      ...
      if (!isStill && currentLeaveHandler) currentLeaveHandler()
    }
    ...
    return () => {
      ...
      if (activeTarget) cleanupTarget(activeTarget)
      spinTl.current?.kill()
      if (hideDefaultCursor) document.body.classList.remove('custom-cursor')
      isActiveRef.current              = false
      targetCornerPositionsRef.current = null
