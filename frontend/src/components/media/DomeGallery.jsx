...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      const overlayRelRoot = { left: currentRect.left - rootRect.left, top: currentRect.top - rootRect.top, width: currentRect.width, height: currentRect.height };
      const origRelRoot = { left: originalPos.left - rootRect.left, top: originalPos.top - rootRect.top, width: originalPos.width, height: originalPos.height };
      const animating = document.createElement('div');
      animating.className = 'enlarge-closing';
      animating.style.cssText = `position:absolute;left:${overlayRelRoot.left}px;top:${overlayRelRoot.top}px;width:${overlayRelRoot.width}px;height:${overlayRelRoot.height}px;z-index:9999;border-radius:var(--enlarge-radius,32px);overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,.35);transition:all ${enlargeTransitionMs}ms ease-out;pointer-events:none;margin:0;transform:none;`;
      const origImg = overlay.querySelector('img');
      if (origImg) { const img = origImg.cloneNode(); img.style.cssText = 'width:100%;height:100%;object-fit:cover;'; animating.appendChild(img); }
      overlay.remove();
      rootRef.current.appendChild(animating);
      void animating.getBoundingClientRect();
      requestAnimationFrame(() => {
        animating.style.left = origRelRoot.left + 'px';
        animating.style.top = origRelRoot.top + 'px';
        animating.style.width = origRelRoot.width + 'px';
        animating.style.height = origRelRoot.height + 'px';
  ...
  const openItemFromElement = useCallback(el => {
    ...
    if (openedImageWidth || openedImageHeight) {
      overlay.addEventListener('transitionend', function onFirst(ev) {
        if (ev.propertyName !== 'transform') return;
        overlay.removeEventListener('transitionend', onFirst);
